...
prompt toward a guideline-based reasoning strategy. It contains no
...
# a guideline-based mutation. The model receives a single parent prompt
# and is instructed to produce exactly one variant that directs the
# answering model to reason according to established clinical guidelines
# and standard medical practice while preserving the parent's underlying
# objective. The resulting text should be suitable for direct processing
# by PromptCleaner and PromptValidator.
...
Your objective is to create exactly ONE new version of the parent prompt that changes the reasoning strategy to guideline-based clinical reasoning.
...
- Instruct the answering model to evaluate candidate answers according to established clinical guidelines, evidence-based standard-of-care, and accepted medical consensus.
- Encourage the answering model to prefer management or diagnostic decisions that are most consistent with current clinical practice.
- Encourage the answering model to recognize when uncommon or outdated practices should be rejected unless explicitly supported by the question.
...
Operator Constraint
-------------------
This mutation MUST produce a guideline-based reasoning prompt.

Do NOT replace the guideline-based reasoning strategy with:
...
Guideline-based reasoning must remain the dominant reasoning philosophy.

Novelty Requirement
-------------------
The generated prompt should be substantially different from the parent.

A human reviewer should immediately recognize that the child prompt follows a guideline-based clinical reasoning philosophy.
